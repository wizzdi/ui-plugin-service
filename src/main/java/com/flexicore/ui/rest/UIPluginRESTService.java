...
@OpenAPIDefinition(tags = {@Tag(name = "UIPlugins", description = "UIPlugins Services")})
@Tag(name = "UIPlugins")
@Extension
@Component
public class UIPluginRESTService implements RestServicePlugin {
	...
	public UIPlugin UIPluginUpdate(
			@HeaderParam("authenticationKey") String authenticationKey,
			UIPluginUpdate uiPluginUpdate,
			@Context SecurityContext securityContext) {
		String id = uiPluginUpdate.getId();
		UIPlugin uiPlugin = id != null ? service.getByIdOrNull(id,
				UIPlugin.class, null, securityContext) : null;
		if (uiPlugin == null) {
			throw new BadRequestException("no UIPlugin with id  " + id);
		}
		uiPluginUpdate.setUIPlugin(uiPlugin);
		service.validate(uiPluginUpdate, securityContext);
